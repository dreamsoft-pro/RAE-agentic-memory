...
class MockMemoryStorage(IMemoryStorage):
    ...
    async def list_memories(
        self, tenant_id: str, agent_id: str | None = None, layer: str | None = None, **kwargs: Any
    ) -> list[dict[str, Any]]:
        async with self._lock:
            ...
            for m in self._memories.values():
                ...
                if agent_id and m["agent_id"] != agent_id: continue
                if layer and m["layer"] != layer: continue
            ...
            # Sorting
            results.sort(key=lambda x: x.get(order_by, 0), reverse=(direction == "desc"))
    ...
    async def delete_memories_with_metadata_filter(
        ...
        for mid, m in self._memories.items():
            if tenant_id and m["tenant_id"] != tenant_id: continue
            if agent_id and m["agent_id"] != agent_id: continue
            if layer and m["layer"] != layer: continue
    ...
    async def delete_memories_below_importance(self, tenant_id: str, agent_id: str, layer: str, importance_threshold: float) -> int:
        ...
        to_delete = []
        for mid, m in self._memories.items():
            if m["tenant_id"] == tenant_id and m["agent_id"] == agent_id and m["layer"] == layer:
        ...
        for m in self._memories.values():
            if m["tenant_id"] == tenant_id and m["agent_id"] == agent_id:
                ...
                if query.lower() in m["content"].lower():
                    results.append({"memory": m.copy(), "score": 1.0, "id": m["id"], "content": m["content"], "importance": m["importance"]})
    ...
    async def update_memory_expiration(self, memory_id: UUID, tenant_id: str, expires_at: datetime | None) -> bool:
        async with self._lock:
            memory = self._memories.get(memory_id)
            if memory and memory["tenant_id"] == tenant_id:
    ...
    async def clear_tenant(self, tenant_id: str) -> int:
        count = 0
        to_delete = [mid for mid, m in self._memories.items() if m["tenant_id"] == tenant_id]
        for mid in to_delete:
            del self._memories[mid]
            count += 1
        return count
