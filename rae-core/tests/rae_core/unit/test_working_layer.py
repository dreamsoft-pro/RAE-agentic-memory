...
async def test_add_memory_capacity_eviction(working_layer):
    ...
    # Verify count
    count = await working_layer.storage.count_memories("test_tenant", "test_agent", "working")
    assert count == 3
    ...
    memories = await working_layer.storage.list_memories("test_tenant", "test_agent", "working")
