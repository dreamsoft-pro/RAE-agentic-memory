...
async def test_reflection_engine_find_low_quality_string_id(mock_storage):
    ...
    # Monkeypatch evaluator to return low quality
    ...
    results = await engine.prune_low_quality_memories("t1", "a1", quality_threshold=0.5)
...
async def test_reflection_evaluator_extra(mock_storage):
    ...
    with patch.object(evaluator.scorer, "evaluate", return_value=MagicMock()) as mock_evaluate:
        await evaluator.evaluate_memory_quality(memory["id"], tenant_id="t1", context="query")
