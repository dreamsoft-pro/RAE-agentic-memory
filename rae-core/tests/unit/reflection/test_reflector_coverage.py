...
class TestReflectorCoverage:
    ...
    async def test_generate_reflection_unknown_type(self, mock_deps):
        ...
        r = Reflector(ms)
        res = await r.generate_reflection([uuid4()], "t", "a", reflection_type="unknown")
        assert res["success"] is False
    ...
    async def test_generate_insight_with_llm(self, mock_deps):
        ms, llm = mock_deps
        ms.get_memory.return_value = {"content": "important content", "importance": 0.9, "layer": "em"}
        r = Reflector(ms, llm_provider=llm)
        res = await r.generate_reflection([uuid4()], "t", "a", reflection_type="insight")
