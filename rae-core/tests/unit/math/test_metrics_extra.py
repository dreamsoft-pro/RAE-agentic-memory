...
def test_quality_scorer_extra():
    ...
    scorer = QualityScorer([m1, m2], weights={"text_coherence": 2.0, "shannon_entropy": 2.0})
    ...
    # Test with 0 weight
    scorer = QualityScorer([m1, m2], weights={"text_coherence": 1.0, "shannon_entropy": 0.0})
    res = scorer.evaluate("test")
    assert "shannon_entropy" not in res.metadata["components"]
    
    # Test with total weight 0
    scorer = QualityScorer([m1, m2], weights={"text_coherence": 0.0, "shannon_entropy": 0.0})
    res = scorer.evaluate("test")
    assert res.score == 0.0
