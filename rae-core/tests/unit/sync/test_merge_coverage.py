...
class TestMergeCoverage:
    ...
    def test_resolve_keep_local(self, resolver):
        ...
        res = resolver.resolve(local, remote, ["content"], strategy=ConflictResolutionStrategy.KEEP_LOCAL)
        assert res.success
    ...
    def test_resolve_keep_remote(self, resolver):
        local = {"id": str(uuid4()), "content": "local"}
        remote = {"id": local["id"], "content": "remote"}
        res = resolver.resolve(local, remote, ["content"], strategy=ConflictResolutionStrategy.KEEP_REMOTE)
        assert res.success
    ...
    def test_resolve_manual_strategy(self, resolver):
        local = {"id": str(uuid4()), "content": "local"}
        remote = {"id": local["id"], "content": "remote"}
        res = resolver.resolve(local, remote, ["content"], strategy=ConflictResolutionStrategy.MANUAL)
    ...
    def test_merge_memories_flow(self):
        ...
        local = [
            ...
            {"id": str(mid3), "content": "local conflict", "modified_at": datetime.now(timezone.utc)}
        ]
        remote = [
            {"id": str(mid2), "content": "only remote"},
            {"id": str(mid3), "content": "remote conflict", "modified_at": datetime.now(timezone.utc) + timedelta(minutes=1)}
        ]
    ...
    def test_apply_merge_results_success(self):
        mid = uuid4()
        res = MergeResult(
            memory_id=mid, 
            success=True, 
        ...
        summary = apply_merge_results([res], AsyncMock())
