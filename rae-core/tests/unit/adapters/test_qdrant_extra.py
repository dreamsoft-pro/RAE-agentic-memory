...
async def test_search_with_contradiction_penalty_missing_vectors():
    ...
    with patch.object(store, "search_similar", AsyncMock(return_value=[(id1, 0.9), (id2, 0.8), (id3, 0.7)])):
        # Mock get_vector to return vector for id1 and id3, but NOT for id2
        # This will cover lines 470 and 474
        with patch.object(store, "get_vector", AsyncMock(side_effect=lambda memory_id, tenant_id: [1.0] if memory_id != id2 else None)):
...
async def test_qdrant_init_with_url():
    ...
    with patch("rae_core.adapters.qdrant.AsyncQdrantClient") as mock_client_cls:
        store = QdrantVectorStore(url="http://localhost:6333", api_key="test-key")
        mock_client_cls.assert_called_once_with(url="http://localhost:6333", api_key="test-key")
...
async def test_search_with_contradiction_penalty_low_results():
    ...
    with patch.object(store, "search_similar", AsyncMock(return_value=[(uuid4(), 0.9)])):
...
async def test_search_with_contradiction_penalty_full_logic():
    ...
    # Mock search_similar
    with patch.object(store, "search_similar", AsyncMock(return_value=[(id1, 0.9), (id2, 0.8)])):
        # Mock get_vector to return contradictory vectors (e.g. opposite)
        # Similarity should be < 0.15
        with patch.object(store, "get_vector", AsyncMock(side_effect=[[1.0, 0.0], [-1.0, 0.0]])):
