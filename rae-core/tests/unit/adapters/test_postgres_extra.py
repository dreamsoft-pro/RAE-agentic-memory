...
async def test_get_metric_aggregate_coverage(mock_pool):
    ...
    # Test with filters
    await storage.get_metric_aggregate("t1", "importance", "avg", filters={"agent_id": "a1"})
...
async def test_delete_memories_with_metadata_filter_coverage(mock_pool):
    ...
    conn.execute.return_value = "DELETE 1"
    count = await storage.delete_memories_with_metadata_filter("t1", "a1", "episodic", {"key": "val"})
    assert count == 1
    
    # Case where num__lt matches
    conn.execute.return_value = "DELETE 1"
    count = await storage.delete_memories_with_metadata_filter("t1", "a1", "episodic", {"num__lt": 15})
    assert count == 1
    
    # Case where no matches (to cover 653-654)
    count = await storage.delete_memories_with_metadata_filter("t1", "a1", "episodic", {"num__lt": 5})
    assert count == 0
...
async def test_adjust_importance_not_found(mock_pool):
    ...
    with pytest.raises(ValueError, match="Memory not found"):
        await storage.adjust_importance("t1", uuid4(), 0.9)