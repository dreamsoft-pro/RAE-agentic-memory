...
@pytest.mark.asyncio
async def test_engine_init_with_cache_and_sync(mock_storage, mock_vector_store, mock_embedding_provider, mock_cache_provider, mock_sync_provider):
...
async def test_store_memory_with_embedding_manager(mock_storage, mock_vector_store):
    ...
    mock_provider.generate_all_embeddings = AsyncMock(return_value={
        "default": [[0.1] * 128],
        "other": [[0.2] * 128]
    })
...
@pytest.mark.asyncio
async def test_search_memories_with_reranker(mock_storage, mock_vector_store, mock_embedding_provider):
    ...
    results = await engine.search_memories("query", "t1", agent_id="a1", layer="episodic", use_reranker=True)
    
    assert len(results) == 1
    assert results[0]["search_score"] == 0.9
    assert engine.search_engine.rerank.called

@pytest.mark.asyncio
async def test_generate_reflection(mock_storage, mock_vector_store, mock_embedding_provider):
    engine = RAEEngine(mock_storage, mock_vector_store, mock_embedding_provider)
    engine.reflection_engine.generate_reflection = AsyncMock(return_value={"status": "ok"})
    
    result = await engine.generate_reflection([uuid4()], "t1", "a1")
    assert result == {"status": "ok"}

@pytest.mark.asyncio
async def test_sync_memories(mock_storage, mock_vector_store, mock_embedding_provider, mock_sync_provider):
    settings = RAESettings(sync_enabled=True)
    engine = RAEEngine(mock_storage, mock_vector_store, mock_embedding_provider, settings=settings, sync_provider=mock_sync_provider)
...
@pytest.mark.asyncio
async def test_sync_memories_disabled(mock_storage, mock_vector_store, mock_embedding_provider):
    engine = RAEEngine(mock_storage, mock_vector_store, mock_embedding_provider)
    result = await engine.sync_memories("t1", "a1")
    assert result is None

@pytest.mark.asyncio
async def test_generate_text_no_orchestrator(mock_storage, mock_vector_store, mock_embedding_provider):
    engine = RAEEngine(mock_storage, mock_vector_store, mock_embedding_provider)
