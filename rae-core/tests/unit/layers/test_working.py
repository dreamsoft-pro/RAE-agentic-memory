...
@pytest.mark.asyncio
async def test_add_memory_exceeding_capacity_triggers_eviction(working_layer, mock_storage):
    expected_id = uuid4()
    mock_storage.store_memory.return_value = expected_id
    ...
    mock_storage.list_memories.return_value = [{"id": least_important_id, "importance": 0.1}]
...
async def test_get_memory_updates_access(working_layer, mock_storage):
    ...
    mock_storage.get_memory.return_value = {
        "id": mem_id, "content": "active", "layer": "working",
