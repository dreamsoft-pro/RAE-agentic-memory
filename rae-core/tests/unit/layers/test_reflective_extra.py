...
async def test_reflective_layer_search_memories(mock_storage):
    ...
    mock_storage.search_memories.return_value = [
        {"memory": {"id": uuid4(), "content": "c1", "layer": "reflective", "tenant_id": "t1", "agent_id": "a1", "importance": 0.9, "created_at": datetime.now().isoformat()}, "score": 0.9}
    ]
    
    results = await layer.search_memories("query", reflection_type=ReflectionType.INSIGHT)
