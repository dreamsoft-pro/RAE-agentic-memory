...
class DummyLayer(MemoryLayerBase):
    async def add_memory(self, content, tags=None, metadata=None, embedding=None, importance=None):
        await super().add_memory(content, tags, metadata, embedding, importance)
        return uuid4()
        
    async def get_memory(self, memory_id):
        await super().get_memory(memory_id)
        return None
        
    async def search_memories(self, query, limit=10, filters=None):
        await super().search_memories(query, limit, filters)
        return []
