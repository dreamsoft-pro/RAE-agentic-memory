...
@pytest.mark.asyncio
async def test_add_reflection_insufficient_source_memories(reflective_layer, mock_storage):
    reflection = Reflection(
        content="insight",
        reflection_type=ReflectionType.INSIGHT,
    ...
    with pytest.raises(ValueError, match="Reflection requires at least 2 source memories"):
...
async def test_get_reflections_by_type(reflective_layer, mock_storage):
    ...
    mock_storage.list_memories.return_value = [
        {
            "id": mem_id_1, "content": "pattern 1", "layer": "reflective",
            "tenant_id": "t", "agent_id": "a", "created_at": now, "last_accessed_at": now,
            "metadata": {"reflection_type": "pattern", "priority": "high", "source_memory_ids": []}
        },
        {
            "id": mem_id_2, "content": "pattern 2", "layer": "reflective",
            "tenant_id": "t", "agent_id": "a", "created_at": now, "last_accessed_at": now,
            "metadata": {"reflection_type": "pattern", "priority": "medium", "source_memory_ids": []}
        }
    ]
