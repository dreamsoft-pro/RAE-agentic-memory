...
class SQLiteStorage(IMemoryStorage):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute(
                ...
                (str(m_id), kwargs.get("content"), kwargs.get("layer"), kwargs.get("tenant_id"), 
                 kwargs.get("agent_id"), json.dumps(kwargs.get("tags", [])), json.dumps(kwargs.get("metadata", {})),
    ...
    async def update_memory(self, memory_id: UUID, tenant_id: str, updates: dict[str, Any]) -> bool:
        await self.initialize()
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute("UPDATE memories SET content = ? WHERE id = ? AND tenant_id = ?", (updates.get("content"), str(memory_id), tenant_id))
            await db.commit()
        return True
    ...
    async def delete_memory(self, memory_id: UUID, tenant_id: str) -> bool:
        await self.initialize()
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute("DELETE FROM memories WHERE id = ? AND tenant_id = ?", (str(memory_id), tenant_id))
            await db.commit()
        return True
    ...
    async def list_memories(self, tenant_id: str, **kwargs: Any) -> list[dict[str, Any]]:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            db.row_factory = aiosqlite.Row
            async with db.execute("SELECT * FROM memories WHERE tenant_id = ? LIMIT ?", (tenant_id, limit)) as cursor:
