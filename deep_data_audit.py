...
async def deep_audit():
    ...
    for col in columns:
        ...
        count_null = await conn.fetchval(f"SELECT COUNT(*) FROM memories WHERE {col} IS NULL")
        total = await conn.fetchval("SELECT COUNT(*) FROM memories")
        print(f"Column '{col:.<15}' | NULLs: {count_null:<6} | Fill Rate: {((total-count_null)/total)*100:>.1f}%")

    print("\n=== DATA INTEGRITY AUDIT: ACTIVE WRITERS ===")
    # Sprawdzamy czy tabele pomocnicze są w ogóle używane
    tables = ['access_logs', 'token_savings_log', 'knowledge_graph_nodes', 'memory_embeddings']
    for table in tables:
