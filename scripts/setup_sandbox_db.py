...
async def setup_sandbox():
    ...
    try:
        ...
        exists = await conn.fetchval("SELECT EXISTS(SELECT 1 FROM tenants WHERE id = $1)", t_id)
        if not exists:
            print(f"ℹ️ Creating default tenant {DEFAULT_TENANT_ID}...")
            await conn.execute("INSERT INTO tenants (id, name, tier) VALUES ($1, $2, 'enterprise')", t_id, "Sandbox Tenant")
        ...
        """, uuid4(), API_KEY_USER_ID, t_id, ROLE, [], "sandbox-setup")
