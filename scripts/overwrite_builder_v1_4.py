...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-3 mt-2">
        <h4><span class="text-primary">Flex</span>Charts <small class="text-muted" style="font-size: 0.6em">v1.4 Fix</small></h4>
...
<script>
    ...
    function getRangeLabel(config) {
        if(config.range === 'custom') return 'Custom';
        const labels = {
            '30s': '30s', '1m': '1m', '5m': '5m', '15m': '15m',
            '1h': '1h', '8h': 'Shift', '24h': '24h', '7d': '7d'
        };
        return labels[config.range] || config.range;
    }
            ...
            <div class="grid-stack-item-content" data-chart-type="${w.widget_type}">
                <div class="widget-header">
                    <div class="text-truncate" style="max-width: 80%;">
                        <span id="title-${w.id}">${w.title}</span>
                        <small class="text-muted fw-normal ms-2" style="font-size:0.75em" id="range-label-${w.id}">${rangeLabel}</small>
                    </div>
            ...
            fetch(`/dashboard/api/series/?machine_id=${machineId}&metric=${metric}&from=${isoStart}&to=${isoEnd}`)
                ...
                .then(res => {
                    ...
                    const option = {
                        ...
                        grid: { 
                            left: 15, right: 35, // Increased margins
                            bottom: config.show_zoom ? 50 : 15, top: 35, 
                            containLabel: true 
                        },
                        ...
                        yAxis: { 
                            type: 'value', 
                            scale: true,
                            axisLabel: { hideOverlap: true } // Hide overlapping labels
                        },
                        dataZoom: config.show_zoom ? [{ 
                            type: 'slider', bottom: 5, height: 35, handleSize: '150%' 
                        }] : [],
                        series: [{
                            name: metric,
                            type: 'line',
                            smooth: type === 'production_trend' ? 0.2 : true,
                            symbol: 'none',
                            data: data.map(d => [new Date(d.t).getTime(), d.v]),
                            lineStyle: { width: 1.5 },
    ...
    // Modal & Settings functions remain the same...
    function openGlobalSettings() { new bootstrap.Modal(document.getElementById('globalSettingsModal')).show(); }
    function applyGlobalSettings() {
        ...
        Object.keys(widgetConfigs).forEach(id => {
            ...
            if (range === 'custom') { widgetConfigs[id].custom_from = from; widgetConfigs[id].custom_to = to; }
    ...
    function saveWidgetSettings() {
        ...
        widgetConfigs[id].range = document.getElementById('editRange').value;
        widgetConfigs[id].show_zoom = document.getElementById('editZoom').checked;
        if (widgetConfigs[id].range === 'custom') {
        ...
        document.getElementById('title-'+id).innerText = document.getElementById('editTitle').value;
        document.getElementById('range-label-'+id).innerText = getRangeLabel(widgetConfigs[id]);
        initChart(id, widgetTypes[id]);
...
print(f"Overwritten {target_path} with v1.4 Fix")
