...
async def recover_memory():
    ...
    try:
        ...
        # 1. em -> episodic
        res = await conn.execute("UPDATE memories SET layer = 'episodic' WHERE layer = 'em'")
        print(f" - Recovered episodic memories (em -> episodic): {res}")
        
        # 2. stm/wm -> working
        res = await conn.execute("UPDATE memories SET layer = 'working' WHERE layer = 'stm' OR layer = 'wm'")
        print(f" - Recovered working memories (stm/wm -> working): {res}")
        
        # 3. ltm/sm -> semantic
        res = await conn.execute("UPDATE memories SET layer = 'semantic' WHERE layer = 'ltm' OR layer = 'sm'")
        print(f" - Recovered semantic memories (ltm/sm -> semantic): {res}")
        
        # 4. rm -> reflective
        res = await conn.execute("UPDATE memories SET layer = 'reflective' WHERE layer = 'rm'")
