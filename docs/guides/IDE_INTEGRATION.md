...
vnoremap <leader>rs :!curl -X POST http://localhost:8000/v2/memory/store \
...
M.save_memory = function()
  ...
  local cmd = string.format(
    'curl -s -X POST http://localhost:8000/v2/memory/store ' ..
    '-H "Content-Type: application/json" ' ..
    '-H "X-API-Key: your-api-key" ' ..
    '-H "X-Project-Id: my-project" ' ..
    '-H "X-Tenant-Id: my-tenant" ' ..
...
```json
{
  "shell_cmd": "curl -X POST http://localhost:8000/v2/memory/store -H 'Content-Type: application/json' -H 'X-API-Key: your-api-key' -H 'X-Project-Id: my-project' -H 'X-Tenant-Id: my-tenant' -d '{\"content\": \"$selection\", \"source\": \"$file\", \"layer\": \"episodic\"}'",
  "selector": "source"
}
```
