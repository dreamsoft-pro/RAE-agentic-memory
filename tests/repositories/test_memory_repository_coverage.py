...
async def test_insert_memory_success(mock_pool):
    ...
    result = await repo.insert_memory(
        tenant_id="t1", content="content", source="user", importance=0.5,
        layer="em", tags=["tag"], timestamp=datetime.now(), project="p1"
    )
...
async def test_insert_memory_failure(mock_pool):
    ...
    result = await repo.insert_memory(
        tenant_id="t1", content="content", source="user", importance=0.5,
        layer="em", tags=None, timestamp=datetime.now(), project="p1"
    )
