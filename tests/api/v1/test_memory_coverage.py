...
from apps.memory_api.api.v1.memory import get_hybrid_search_service, get_vector_store, get_embedding_service, MemoryRepository
...
async def test_store_memory_vector_failure(client_with_auth, mock_memory_repo, mock_embedding_service, mock_vector_store, mock_pii_scrubber):
    mock_memory_repo.insert_memory.return_value = {
        "id": str(uuid4()), "created_at": datetime.now(), "last_accessed_at": None, "usage_count": 0
    }
    ...
    response = client_with_auth.post("/v1/memory/store", json=payload, headers={"X-Tenant-Id": "t1"})
    ...
    if "detail" in data:
        ...
        if isinstance(error_content, dict):
            assert "Vector store error" in error_content.get("message", str(error_content))
        else:
...
async def test_query_memory_hybrid_missing_project(client_with_auth):
    ...
    response = client_with_auth.post("/v1/memory/query", json=payload, headers={"X-Tenant-Id": "t1"})
...
async def test_delete_memory_success(client_with_auth, mock_memory_repo, mock_vector_store):
    ...
    response = client_with_auth.delete("/v1/memory/delete", params={"memory_id": "mem-1"}, headers={"X-Tenant-Id": "t1"})
...
async def test_rebuild_reflections(client_with_auth):
    with patch("apps.memory_api.api.v1.memory.generate_reflection_for_project") as mock_task:
        ...
        response = client_with_auth.post("/v1/memory/rebuild-reflections", json=payload, headers={"X-Tenant-Id": "t1"})
...
async def test_get_reflection_stats(client_with_auth, mock_memory_repo):
    ...
    response = client_with_auth.get("/v1/memory/reflection-stats", params={"project": "p1"}, headers={"X-Tenant-Id": "t1"})
