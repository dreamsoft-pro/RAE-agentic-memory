...
from apps.memory_api.routes.dashboard import router, get_metric_timeseries
from apps.memory_api.models.dashboard_models import (
    ...
    GetVisualizationRequest, VisualizationType, GetAuditTrailRequest
)
...
async def test_compliance_report(mock_compliance_service):
    ...
    req = GetComplianceReportRequest(tenant_id="t1", project_id="p1", report_type="full")
...
async def test_risk_register(mock_compliance_service):
    ...
    mock_risk = RiskMetric(
        risk_id="r1", risk_description="desc", category="cat", probability=0.5, impact=0.5,
        risk_score=0.25, risk_level=RiskLevel.HIGH, status="open",
        identified_at=datetime.now(timezone.utc), last_reviewed_at=datetime.now(timezone.utc)
    )
    
    mock_compliance_service._get_active_risks.return_value = [mock_risk]
    req = GetRiskRegisterRequest(tenant_id="t1", project_id="p1", risk_level=RiskLevel.HIGH)
...
async def test_get_metric_timeseries_trend_up(mock_metrics_repo):
    ...
    result = await get_metric_timeseries("memory_count", "t1", "p1", MetricPeriod.LAST_24H, mock_metrics_repo)
...
async def test_get_visualization_reflection_tree(mock_pool):
    ...
    root_record = {
        "id": uuid4(), "content": "Root", "type": "meta", "score": 1.0, 
        "depth_level": 0, "parent_reflection_id": None, "cluster_id": None, 
        "source_count": 5, "created_at": datetime.now(timezone.utc)
    }
    ...
    mock_pool.fetch.side_effect = [
        [root_record], # Root search
        []             # Children search
    ]
    
    req = GetVisualizationRequest(tenant_id="t1", project_id="p1", visualization_type=VisualizationType.REFLECTION_TREE)
...
async def test_get_visualization_semantic_graph(mock_pool):
    ...
    # Nodes
    nodes = [{
        "id": uuid4(), "label": "Node1", "node_type": "concept", "canonical_form": "node1",
        "importance_score": 0.8, "reinforcement_count": 1, "is_degraded": False
    }]
    # Edges
    edges = [{
        "source_node_id": nodes[0]["id"], "target_node_id": uuid4(), "relation_type": "related",
        "edge_weight": 1.0, "confidence": 0.9
    }]
    
    mock_pool.fetch.side_effect = [nodes, edges]
    
    req = GetVisualizationRequest(tenant_id="t1", project_id="p1", visualization_type=VisualizationType.SEMANTIC_GRAPH)
    resp = await get_visualization(req, mock_pool)
...
async def test_get_visualization_unsupported(mock_pool):
    ...
    req = GetVisualizationRequest(tenant_id="t1", project_id="p1", visualization_type=VisualizationType.CLUSTER_MAP)
    
    with pytest.raises(HTTPException) as exc:
        await get_visualization(req, mock_pool)
    
    assert exc.value.status_code == 400 # Or 500 depending on implementation details, likely 400
...
async def test_get_rls_status(mock_compliance_service):
    ...
    resp = await get_rls_status(tenant_id="t1", compliance_service=mock_compliance_service)
