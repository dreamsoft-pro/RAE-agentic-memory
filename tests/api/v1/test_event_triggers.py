...
async def test_create_trigger_success(mock_trigger_repo):
    ...
    req = CreateTriggerRequest(
        ...
        actions=[
            ActionConfig(action_type=ActionType.SEND_NOTIFICATION, config={"msg": "High importance"})
        ],
        created_by="user1"
    )
...
async def test_create_trigger_failure(mock_trigger_repo):
    ...
    req = CreateTriggerRequest(
        tenant_id="t1", project_id="p1", rule_name="Test",
...
async def test_list_triggers(mock_trigger_repo):
    ...
    mock_triggers = [{"id": uuid4(), "rule_name": "T1"}, {"id": uuid4(), "rule_name": "T2"}]
...
async def test_create_workflow_success(mock_workflow_repo):
    ...
    step1 = WorkflowStep(
        step_id="s1", step_name="Step 1", order=1,
        action=ActionConfig(action_type=ActionType.SEND_NOTIFICATION, config={})
    )
    
    req = CreateWorkflowRequest(
        tenant_id="t1", project_id="p1", workflow_name="WF1",
        steps=[step1], created_by="u1"
    )
...
async def test_instantiate_template_success(mock_pool):
    ...
    with patch.dict("apps.memory_api.routes.event_triggers.DEFAULT_TEMPLATES", {
        template_id: TriggerTemplate(
            template_id=template_id, template_name="Test", description="Desc",
            category="test", parameters={}, required_parameters=["p1"], 
            use_cases=[],
            default_condition=TriggerCondition(event_types=[EventType.MEMORY_CREATED]),
            default_actions=[ActionConfig(action_type=ActionType.SEND_NOTIFICATION)]
        )
    }):
        req_params = {"p1": "value"}
