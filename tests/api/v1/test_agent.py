...
@pytest.fixture
def mock_services():
    with patch("apps.memory_api.api.v1.agent.get_embedding_service") as mock_embed, \
         ...
         patch("apps.memory_api.api.v1.agent._update_memory_access_stats", new_callable=AsyncMock):
...
@pytest.mark.asyncio
async def test_agent_execute_happy_path(client_with_auth, mock_context_builder, mock_services):
    ...
    # Mock LLM response
    mock_services["llm"].generate = AsyncMock(return_value=LLMResult(
        text="Agent Answer",
        usage=LLMResultUsage(prompt_tokens=50, candidates_tokens=20, total_tokens=70),
        model_name="gpt-4",
        finish_reason="stop"
    ))
    ...
    with patch("httpx.AsyncClient") as MockClient:
        ...
        # Reranker response
        mock_client_instance.post.return_value.json = AsyncMock(return_value={
            "items": [{"id": mock_item.id, "score": 0.95}]
        })
...
@pytest.mark.asyncio
async def test_agent_execute_no_episodic_memories(client_with_auth, mock_context_builder, mock_services):
    ...
    mock_services["llm"].generate = AsyncMock(return_value=LLMResult(
        ...
        usage=LLMResultUsage(prompt_tokens=10, candidates_tokens=5, total_tokens=15),
        model_name="gpt-4",
        finish_reason="stop"
    ))
    ...
    with patch("httpx.AsyncClient") as MockClient:
        ...
        response = client_with_auth.post("/v1/agent/execute", json={
            "tenant_id": "test-tenant",
            "project": "test-project",
            "prompt": "Query"
        })
...
@pytest.mark.asyncio
async def test_agent_execute_reranker_failure(client_with_auth, mock_context_builder, mock_services):
...
@pytest.mark.asyncio
async def test_agent_execute_llm_failure(client_with_auth, mock_context_builder, mock_services):
...
@pytest.mark.asyncio
async def test_agent_execute_reflection_failure_ignored(client_with_auth, mock_context_builder, mock_services):
...
@pytest.mark.asyncio
async def test_agent_execute_vector_store_failure(client_with_auth, mock_context_builder, mock_services):
