...
async def test_agent_execute_direct(mock_app_state_pool):
    with patch(
        "apps.memory_api.api.v1.agent.get_context_cache"
        ...
        "apps.memory_api.api.v1.agent._update_memory_access_stats", new=AsyncMock()
    ), patch(
        "httpx.AsyncClient"
    ) as mock_httpx:
        ...
        mock_llm_inst.generate = AsyncMock(
            return_value=LLMResult(
                text="Direct Answer",
                usage=LLMResultUsage(
                    prompt_tokens=10, candidates_tokens=10, total_tokens=20
                ),
                model_name="gpt-4",
                finish_reason="stop",
            )
        ...
        payload = AgentExecuteRequest(tenant_id="t1", project="p1", prompt="Hi")
    ...
    with patch(
        ...
        mock_llm_inst.generate = AsyncMock(
            return_value=LLMResult(
                ...
                usage=LLMResultUsage(
                    prompt_tokens=20, candidates_tokens=15, total_tokens=35
                ),
                model_name="gpt-4",
                finish_reason="stop",
            )
        ...
        payload = AgentExecuteRequest(
            tenant_id="t1", project="p1", prompt="Set UI theme"
        )
    ...
    with patch(
        ...
        mock_llm_inst.generate = AsyncMock(
            return_value=LLMResult(
                ...
                usage=LLMResultUsage(
                    prompt_tokens=5, candidates_tokens=5, total_tokens=10
                ),
                model_name="gpt-4",
                finish_reason="stop",
            )
        ...
        payload = AgentExecuteRequest(tenant_id="t1", project="p1", prompt="")
...
async def test_agent_execute_with_llm_error(mock_app_state_pool):
    ...
    with patch(
        ...
        payload = AgentExecuteRequest(tenant_id="t1", project="p1", prompt="test")
