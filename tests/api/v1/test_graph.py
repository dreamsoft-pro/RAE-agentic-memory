...
from apps.memory_api.services.hybrid_search import HybridSearchResult, TraversalStrategy
...
def client_with_auth(mock_hybrid_search, mock_db_pool):
    ...
    with patch("apps.memory_api.main.asyncpg.create_pool", new=AsyncMock(return_value=pool)), \
...
@pytest.mark.asyncio
async def test_generate_hierarchical_reflection(client_with_auth, mock_reflection_engine, mock_db_pool):
    """Test POST /graph/reflection/hierarchical"""
    
    mock_reflection_engine.generate_hierarchical_reflection.return_value = "Summary of episodes"
    ...
    response = client_with_auth.post("/v1/graph/reflection/hierarchical", json=payload, headers=headers)
...
async def test_get_graph_statistics(client_with_auth, mock_db_pool):
    ...
    headers = {"X-Tenant-Id": client_with_auth.tenant_id}
    response = client_with_auth.get("/v1/graph/stats?project_id=test-project", headers=headers)
...
async def test_get_graph_nodes(client_with_auth, mock_db_pool):
    ...
    headers = {"X-Tenant-Id": client_with_auth.tenant_id}
    response = client_with_auth.get("/v1/graph/nodes?project_id=test-project&limit=10", headers=headers)
...
async def test_get_graph_nodes_with_pagerank(client_with_auth, mock_db_pool):
    ...
    with patch("apps.memory_api.services.graph_algorithms.GraphAlgorithmsService") as MockAlgo:
...
async def test_get_graph_edges(client_with_auth, mock_db_pool):
    ...
    headers = {"X-Tenant-Id": client_with_auth.tenant_id}
    response = client_with_auth.get("/v1/graph/edges?project_id=test-project&relation=KNOWS", headers=headers)
...
async def test_error_handling_missing_tenant(client_with_auth):
    ...
    if "detail" in response.json():
        ...
        assert "X-Tenant-Id header is required" in response.json().get("error", {}).get("message", "") or \
               "X-Tenant-Id header is required" in str(response.json())
