...
from apps.memory_api.routes.graph_enhanced import (
    ...
    update_edge_weight, deactivate_edge, activate_edge, set_edge_temporal_validity,
    list_snapshots, get_snapshot, restore_snapshot, get_graph_statistics,
    batch_create_nodes, batch_create_edges, health_check
)
from apps.memory_api.models.graph_enhanced_models import (
    ...
    UpdateEdgeWeightRequest, DeactivateEdgeRequest, ActivateEdgeRequest,
    SetEdgeTemporalValidityRequest, RestoreSnapshotRequest,
    GetGraphStatisticsRequest, GraphStatistics,
...
async def test_update_edge_weight(mock_repo, mock_pool):
    ...
    mock_repo.update_edge_weight.return_value = EnhancedGraphEdge(
        id=edge_id, tenant_id="t1", project_id="p1", source_node_id=uuid4(), target_node_id=uuid4(),
        relation="rel", edge_weight=0.8, confidence=0.9,
...
async def test_traverse_graph(mock_repo, mock_pool):
    req = TraverseGraphRequest(
        tenant_id="t1", project_id="p1", start_node_id=uuid4(),
        algorithm=TraversalAlgorithm.BFS, max_depth=2
    )
...
async def test_detect_cycle(mock_repo, mock_pool):
    ...
    req = DetectCycleRequest(
        tenant_id="t1", project_id="p1", source_node_id=source_id, target_node_id=target_id
    )
    mock_repo.detect_cycle.return_value = CycleDetectionResult(
        has_cycle=True, cycle_length=3,
...
async def test_find_shortest_path(mock_repo, mock_pool):
    ...
    mock_repo.find_shortest_path.return_value = GraphPath(nodes=[], length=0, total_weight=0)
...
async def test_get_graph_statistics(mock_repo, mock_pool):
    ...
    mock_repo.get_graph_statistics.return_value = GraphStatistics(
        ...
        total_nodes=100, total_edges=200, avg_degree=4.0,
        snapshot_count=2, last_snapshot_at=None
    )
...
async def test_create_snapshot(mock_repo, mock_pool):
    ...
    mock_repo.get_snapshot.return_value = GraphSnapshot(
        id=snap_id, tenant_id="t1", project_id="p1", snapshot_name="Snap1",
        node_count=10, edge_count=10, snapshot_size_bytes=100,
        created_at=datetime.now(timezone.utc)
    )
...
async def test_batch_create_nodes(mock_repo, mock_pool):
    req = BatchCreateNodesRequest(tenant_id="t1", project_id="p1", nodes=[{"node_id": "n1", "label": "l1"}])
...
async def test_batch_create_edges(mock_repo, mock_pool):
    edge_req = CreateGraphEdgeRequest(
        tenant_id="t1", project_id="p1", source_node_id=uuid4(), target_node_id=uuid4(), relation="r"
    )
    req = BatchCreateEdgesRequest(tenant_id="t1", project_id="p1", edges=[edge_req])
    
    # Successful creation
    mock_edge = EnhancedGraphEdge(
        id=uuid4(), tenant_id="t1", project_id="p1", source_node_id=uuid4(), target_node_id=uuid4(),
        relation="r", created_at=datetime.now(timezone.utc), updated_at=datetime.now(timezone.utc),
        valid_from=datetime.now(timezone.utc)
    )
    mock_repo.create_edge.return_value = mock_edge
...
async def test_batch_create_edges_partial_failure(mock_repo, mock_pool):
    edge_req1 = CreateGraphEdgeRequest(tenant_id="t1", project_id="p1", source_node_id=uuid4(), target_node_id=uuid4(), relation="r")
    edge_req2 = CreateGraphEdgeRequest(tenant_id="t1", project_id="p1", source_node_id=uuid4(), target_node_id=uuid4(), relation="r")
    req = BatchCreateEdgesRequest(tenant_id="t1", project_id="p1", edges=[edge_req1, edge_req2])
