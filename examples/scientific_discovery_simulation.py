...
class MockRAEClient:
    def __init__(self):
        ...
        self.math_policy = {"alpha": 0.5, "beta": 0.5} # Alpha=Relevance, Beta=Importance
    ...
    async def store_episode(self, action, outcome, metadata):
        ...
        episode = {"action": action, "outcome": outcome, "timestamp": time.time(), "meta": metadata}
    ...
    async def run_reflection_loop(self, last_episode):
        ...
        print(f"   [Math-3] Calculated Surprise Score: {surprise_score:.4f} (Threshold: 0.7)")
...
async def main():
    ...
    console.print(Panel.fit("🧪 RAE Scientific Discovery Simulation", style="bold blue"))

    # Step 1: Hypothesis
    console.print("\n[bold green]1. Hypothesis Generation (Semantic Memory)[/bold green]")
    theory = await client.retrieve("Expected reaction rate for Compound X?", layer="semantic")
    console.print(f"Agent retrieves current theory: [italic]'{theory['theory']}'[/italic]")
    ...
    if insight:
        ...
        console.print("[dim]The agent has autonomousy refined its hypothesis and adjusted its search strategy for future experiments.[/dim]")
