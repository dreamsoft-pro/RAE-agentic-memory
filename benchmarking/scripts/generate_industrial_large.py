...
class IndustrialDataGenerator:
    ...
    def _init_domains(self) -> Dict[str, Dict]:
        ...
        return {
            'logs': {
                ...
                'services': ['api-gateway', 'auth-service', 'db-cluster', 'cache-layer', 'ml-inference'],
                'templates': [
    ...
    def generate_log_entry(self, idx: int) -> Dict:
        ...
        return {
            ...
            'metadata': {
                'source': 'System Logs',
                'importance': 0.3 if level == 'INFO' else 0.6 if level == 'WARN' else 0.9,
    ...
    def generate_ticket_entry(self, idx: int) -> Dict:
        ...
        return {
            ...
            'metadata': {
                ...
                'importance': {'low': 0.3, 'medium': 0.5, 'high': 0.8, 'critical': 0.95}[priority],
                'timestamp': timestamp.isoformat(),
                'type': ticket_type,
                'priority': priority,
                'component': component
    ...
    def generate_incident_entry(self, idx: int) -> Dict:
        ...
        return {
            ...
            'metadata': {
                ...
                'importance': {'sev4': 0.4, 'sev3': 0.6, 'sev2': 0.85, 'sev1': 0.99}[severity],
                'timestamp': timestamp.isoformat(),
                'severity': severity,
    ...
    def generate_queries(self, num_queries: int, memories: List[Dict]) -> List[Dict]:
        ...
        for idx in range(num_queries):
            ...
            expected = random.sample(relevant_memories, num_expected) if relevant_memories else []
            expected_ids = [m['id'] for m in expected]

            difficulty = 'easy' if num_expected <= 1 else 'medium' if num_expected <= 2 else 'hard'
...
def main():
    parser = argparse.ArgumentParser(description='Generate industrial_large.yaml benchmark')
    parser.add_argument('--size', type=int, default=1000, help='Number of memories to generate')
    parser.add_argument('--queries', type=int, default=100, help='Number of queries to generate')
    parser.add_argument('--output', type=str, default='../sets/industrial_large.yaml', help='Output file path')
    parser.add_argument('--seed', type=int, default=42, help='Random seed for reproducibility')

    args = parser.parse_args()

    print(f"🏭 Generating Industrial Large Benchmark")
    ...
    with open(args.output, 'w') as f:
        yaml.dump(benchmark, f, default_flow_style=False, allow_unicode=True, sort_keys=False)
