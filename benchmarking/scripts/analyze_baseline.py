...
def load_all_benchmarks(results_dir: str = "benchmarking/results") -> List[Dict[str, Any]]:
    """Load all academic_lite benchmark results"""
    files = glob.glob(f"{results_dir}/academic_lite_*[0-9].json")
    files = [f for f in files if not any(x in f for x in ['_structure', '_dynamics', '_policy', '_snapshots', '_decisions'])]
...
def analyze_benchmarks(data: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # Math metrics
    gcs_values = [d['metrics']['math']['structure']['graph_connectivity_score']['value'] for d in data]
    orr_values = [d['metrics']['math']['policy']['optimal_retrieval_ratio']['value'] for d in data]
    mdi_values = [d['metrics']['math']['dynamics']['memory_drift_index']['value'] for d in data]
...
if __name__ == "__main__":
    ...
    print(f"\nKey findings:")
    print(f"  MRR: {analysis['quality_metrics']['mrr']['mean']:.3f} ± {analysis['quality_metrics']['mrr']['stdev']:.3f}")
    print(f"  Quality: {analysis['quality_metrics']['overall_quality']['mean']:.3f}")
    print(f"  Query time: {analysis['performance_metrics']['query_time_ms']['mean']:.1f}ms")
