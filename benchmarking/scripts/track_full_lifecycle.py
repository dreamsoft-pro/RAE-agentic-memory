...
class FullLifecycleTracker:
    def __init__(self, benchmark_file: str, kubus_ip: str = None):
    ...
    async def setup(self):
        host = self.kubus_ip if self.kubus_ip else os.getenv("POSTGRES_HOST", "localhost")
    ...
    async def run(self):
        ...
        for i in range(0, len(memories), batch_size):
            ...
            await self.consolidation_service.run_automatic_consolidation(self.tenant_id)

            # 3. Collect stats
            async with self.pool.acquire() as conn:
                rows = await conn.fetch("SELECT layer, COUNT(*) as count FROM memories WHERE tenant_id = $1 GROUP BY layer", self.tenant_id)
    ...
    def plot(self):
        ...
        data = [self.history[l] for l in layers]
        plt.stackplot(self.history["step"], data, labels=layers, colors=colors, alpha=0.8)
        
        plt.title("RAE Full Memory Lifecycle (Warped Time)", fontsize=16, fontweight='bold')
...
async def main():
    tracker = FullLifecycleTracker("benchmarking/sets/industrial_large.yaml", "100.66.252.117")
    await tracker.setup()
    await tracker.run()
    tracker.plot()
