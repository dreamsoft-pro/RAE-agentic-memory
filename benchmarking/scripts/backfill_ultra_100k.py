...
async def backfill_ultra():
    print(f"🚀 Starting PRODUCTION Backfill for Industrial Ultra (100k)")
    ...
    async with httpx.AsyncClient(timeout=180.0) as client:
        for i in range(start_from, total_memories, BATCH_SIZE):
            ...
            for memory, emb in zip(batch, all_embeddings):
                m_id = str(uuid4())
                db_rows.append((m_id, TENANT_ID, memory['text'], memory['id'], 
                               memory.get('metadata', {}).get('importance', 0.5), 1.0, 
                               'semantic', memory.get('tags', []), PROJECT_ID))
                qdrant_points.append(models.PointStruct(id=m_id, vector={"dense": emb},
                    payload={"id": m_id, "tenant_id": TENANT_ID, "content": memory['text'],
                             "source": memory['id'], "project": PROJECT_ID}))
            ...
            try:
                ...
                await qdrant.upsert(collection_name="memories", points=qdrant_points, wait=False)
