...
class TestBenchmarkComparator:
    ...
    def test_improvements_detected(self, tmp_path, sample_baseline_data, sample_improved_data):
        ...
        assert changes['performance']['avg_query_time_ms']['improved'] is True, "Latency should improve"

    def test_regressions_detected(self, tmp_path, sample_baseline_data, sample_regressed_data):
        ...
        assert changes['performance']['avg_query_time_ms']['improved'] is False, "Latency should regress"
    ...
    def test_unchanged_metrics(self, tmp_path, sample_baseline_data):
        ...
        assert changes['quality']['mrr']['percent_change'] == 0.0, "MRR should be unchanged"
        assert changes['quality']['mrr']['absolute_change'] == 0.0, "MRR absolute change should be 0"

    def test_change_percentage_calculation(self, tmp_path, sample_baseline_data, sample_improved_data):
        ...
        assert abs(mrr_change - 13.33) < 0.1, f"MRR change should be ~13.33%, got {mrr_change:.2f}%"

        # Query time: 50.0 → 40.0 = -20%
        latency_change = changes['performance']['avg_query_time_ms']['percent_change']
        assert abs(latency_change - (-20.0)) < 0.1, f"Latency change should be ~-20%, got {latency_change:.2f}%"

    def test_markdown_report_generation(self, tmp_path, sample_baseline_data, sample_improved_data):
        ...
        assert '# RAE Benchmark Comparison Report' in content, "Report should have title"
        ...
        assert 'Performance Metrics' in content, "Report should have performance section"
    ...
    def test_summary_counts(self, tmp_path, sample_baseline_data, sample_improved_data):
        ...
        # Verify counts
        total = (changes['summary']['improvements'] +
                 changes['summary']['regressions'] +
                 changes['summary']['unchanged'])

        assert total > 0, "Should have some metrics counted"
        assert changes['summary']['improvements'] > 0, "Should have improvements in this test data"
    ...
    def test_performance_improvement_detection(self, tmp_path, sample_baseline_data, sample_improved_data):
        ...
        for key in ['avg_query_time_ms', 'p95_query_time_ms']:
            ...
            assert perf_change['comparison'] < perf_change['baseline'], f"{key} should decrease"
            assert perf_change['improved'] is True, f"{key} should be marked as improved"
...
class TestComparisonEdgeCases:
    ...
    def test_missing_metrics_handled(self, tmp_path):
        ...
        baseline_data = {
            ...
            'execution': {'timestamp': '2024-12-01', 'num_memories': 10, 'num_queries': 5, 'total_time_seconds': 5.0},
            'metrics': {
                'mrr': 0.75,
                'hit_rate': {'@5': 0.8},
                'precision': {'@5': 0.7},
