...
def test_lect_pro_target(runner):
    ...
    assert results.consistency_score >= 0.99999, f"LECT failed: {results.consistency_score}"
    assert results.retention_rate >= 0.999, f"LECT retention low: {results.retention_rate}"
...
def test_mmit_pro_target(runner):
    ...
    assert results.interference_score <= 0.002, f"MMIT interference too high: {results.interference_score}"
...
def test_rst_pro_target(runner):
    ...
    assert results.insight_consistency >= 0.66, f"RST consistency low: {results.insight_consistency}"
...
def test_mpeb_pro_target(runner):
    ...
    assert results.adaptation_score >= 0.90, f"MPEB adaptation low: {results.adaptation_score}"
...
def test_orb_pro_target(runner):
    ...
    pareto_count = len([p for p in results.pareto_frontier if p.get("is_pareto_optimal")])
