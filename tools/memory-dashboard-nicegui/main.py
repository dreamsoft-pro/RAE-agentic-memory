...
class AppState:
    def __init__(self):
        self.client = RAEClient(DEFAULT_API_URL, DEFAULT_API_KEY, DEFAULT_TENANT, DEFAULT_PROJECT)
...
class AuditDialog(ui.dialog):
    def __init__(self, memory_id: str, current_content: str, current_tags: List[str], on_confirm):
        super().__init__()
        self.memory_id = memory_id
        self.on_confirm = on_confirm
        
        with self, ui.card().classes('w-full max-w-2xl'):
            ui.label('🛡️ ISO 27001 Compliance Edit').classes('text-xl font-bold text-primary')
            ui.label('All changes are audited. Please provide a valid reason.').classes('text-sm text-gray-500 mb-4')
            
            with ui.row().classes('w-full gap-4 mb-4 bg-gray-50 p-2 rounded'):
                ui.input('Memory ID', value=memory_id).props('readonly').classes('w-1/3')
                ui.input('Timestamp', value=datetime.now().isoformat()).props('readonly').classes('w-1/3')
            
            self.content_input = ui.textarea('Content', value=current_content).classes('w-full').props('outlined')
            self.tags_input = ui.input('Tags (comma separated)', value=",".join(current_tags)).classes('w-full').props('outlined')
            
            ui.separator().classes('my-4')
            
            ui.label('Audit Log (Mandatory)').classes('font-bold')
            self.reason_input = ui.select(
                ['Data Correction', 'PII Removal', 'Context Clarification', 'Erroneous Entry'], 
                label='Reason Code'
            ).classes('w-full')
            self.justification_input = ui.input('Detailed Justification').classes('w-full').props('outlined required')
            
            with ui.row().classes('w-full justify-end mt-4'):
                ui.button('Cancel', on_click=self.close).props('flat color=grey')
                ui.button('AUDIT & SAVE', on_click=self.save).props('color=red icon=security')

    async def save(self):
        if not self.reason_input.value or not self.justification_input.value:
            ui.notify('Reason and Justification are MANDATORY for ISO compliance.', type='negative')
            return
        ...
        await self.on_confirm(self.memory_id, self.content_input.value, new_tags, full_reason)
        self.close()
...
async def main_page():
    ...
    with ui.left_drawer(value=True).classes('bg-slate-50'):
        ...
        tenant_select = ui.select(options=[state.current_tenant], label='Tenant', value=state.current_tenant).classes('w-full')
        # Removed bind_options due to AttributeError
        
        project_select = ui.select(options=[state.current_project], label='Project', value=state.current_project).classes('w-full')
        # Removed bind_options due to AttributeError
        ...
        api_url_input = ui.input('API URL', value=state.client.api_url).classes('w-full')
            ...
            with ui.tab_panel(tab_overview):
                ...
                with ui.row().classes('w-full gap-4 h-full mt-4'):
                    ...
                    with ui.card().classes('w-2/3 h-full'):
                        ui.label('🔍 Search & ISO Edit').classes('font-bold text-lg mb-2')
                        with ui.row().classes('w-full gap-2'):
                            ...
                            async def perform_search():
                                ...
                                results = await state.client.get_recent_memories(limit=20) # TODO: Use real search with query
                                search_results_container.clear()
                                with search_results_container:
                                    for res in results:
                                        with ui.row().classes('w-full items-center justify-between p-2 hover:bg-gray-100 border-b'):
                                            ui.label(res.get('content', '')[:60]).classes('flex-grow')
                                            with ui.row():
                                                ui.chip(res.get('layer', 'N/A')).props('dense')
                                                ui.button(icon='edit', on_click=lambda r=res: open_audit_dialog(r)).props('flat dense round')
                            
                            ui.button('Search', icon='search', on_click=perform_search)
                        
                        search_results_container = ui.column().classes('w-full mt-4 h-96 overflow-y-auto')

            # --- TIMELINE TAB ---
            with ui.tab_panel(tab_timeline):
                ui.label('Coming Soon: Timeline Visualization').classes('text-2xl text-gray-400')
                
            # --- GRAPH TAB ---
            with ui.tab_panel(tab_graph):
                ui.label('Coming Soon: 3D Knowledge Graph').classes('text-2xl text-gray-400')

            # --- QUERY LAB TAB ---
            with ui.tab_panel(tab_query):
                ui.label('Query Laboratory (Szubar Mode Analysis)').classes('text-xl font-bold')
    ...
    async def local_refresh(full_reload=False):
        ...
        if state.is_connected:
            if full_reload or not state.tenants:
                ...
                state.tenants = [t['id'] for t in raw_tenants] if raw_tenants else ['default']
            ...
            with feed_container:
                for mem in state.recent_memories:
                    with ui.card().classes('w-full p-2 bg-gray-50 text-xs'):
                        ui.label(mem.get('content', '')[:80] + '...').classes('font-bold')
    ...
    async def handle_iso_update(mid, content, tags, reason):
        success = await state.client.update_memory(mid, content, tags, reason, "dashboard-operator")
        if success:
