...
def calculate_ndcg(retrieved_items: List[Dict], expected_results: List[Dict], k: int) -> float:
    """
    ...
    expected_relevance_map = {item['source_id']: item['relevance'] for item in expected_results}
    ...
    ideal_relevances = sorted([item['relevance'] for item in expected_results], reverse=True)[:k]
    idcg = 0.0
...
def add_memories(memories):
    ...
    for mem in memories:
        ...
        try:
            ...
            print(f"  - Failed to add memory with source_id: {mem.get('source_id')}. Error: {e}")
...
def run_evaluation(queries):
    ...
    for q in queries:
        ...
        try:
            ...
            # Hit Rate @ 5
            expected_source_ids = {item['source_id'] for item in expected_results if item['relevance'] > 0}
            got_source_ids_top_5 = {item.get("source_id") for item in retrieved_items[:5] if item.get("source_id")}
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run evaluation for the Agentic Memory API.")
    parser.add_argument("--no-add", action="store_true", help="Skip adding memories and run only the evaluation.")
