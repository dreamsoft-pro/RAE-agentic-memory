...
def main():
    ...
    RAE_API_KEY = os.environ.get("RAE_API_KEY", "test-key") # Use the key you've configured in your API
    ...
    api_client = _RAEAPIClient(tenant_id=RAE_TENANT_ID, api_key=RAE_API_KEY, base_url=RAE_API_URL)
...
"""
    ...
    try:
        ...
        print(f"Could not connect to or store memory in RAE API at {RAE_API_URL}. Is it running?")
        print(f"Error: {e}")
        return
    ...
    for doc in retrieved_docs:
        print(f"- Source: {doc.metadata.get('source')}, Content: '{doc.page_content[:50]}...'")
    print("----------------------------")
