...
def main():
    ...
    if not RAE_API_KEY:
        print("RAE_API_KEY environment variable not set. Please set it to your API key.")
        return
    ...
    try:
        vector_store = RAEVectorStore(tenant_id=RAE_TENANT_ID, api_key=RAE_API_KEY, base_url=RAE_API_URL)
    ...
    if os.environ.get("OPENAI_API_KEY"):
        ...
        print("OPENAI_API_KEY not set. Query engine will use retriever only, not an LLM.")
    ...
    documents = [
        Document(text="The project codenamed 'Phoenix' is scheduled for release next month.", metadata={"source": "meeting-notes.txt"}),
        Document(text="The budget for project 'Phoenix' has been increased by 20%.", metadata={"source": "budget-approval.eml"}),
    ]
    ...
    for node in response.source_nodes:
        print(f"- Score: {node.score:.4f}, Source: {node.metadata.get('source')}, Text: '{node.text[:50]}...'")
    print("----------------------------")
