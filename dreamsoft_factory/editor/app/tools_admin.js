...
export var tools = function( editor ){
	...
	function updateCompoundBox(){
		...
		if( _edited ){
            ...
            compoundBox.graphics.c().ss(1/Editor.getStage().scaleX).s("#a8a8a8").mt(
                globalBounds.x,
                globalBounds.y
            ).lt(
			...
			compoundBox.centerPoint = {
				x: nearestVertexX - (( nearestVertexX-farestVertexX  )/2),
				y: nearestVertexY - ( ( nearestVertexY- farestVertexY))/2}
                */
		}
		else {
			//container.visible = false;
	...
	function initTools(){
		...
		if( _edited instanceof EditableArea ){
		...
		var bounds = _edited.getBounds();
				...
				switch( toolsToAttach.components[i].positionInfo ){
					...
					default:
						...
						if( toolsToAttach.components[i].positionInfo instanceof Object ){
							...
							toolsToAttach.components[i].setPosition( _edited.width/2,(( _edited.scaleY < 0  ) ? 40 : -40 ) * 1/_edited.scaleY );
			...
			//**
			//*Ramka poza obrazkiem ; ramka zewnętrzna ; ramka szara ; ramka z alpha ;
			//**
		...
		if( object != edited ){
			...
			if( obj ){
				...
				if( obj.layerElement ){
					...
					if( !$( obj.layerElement ).hasClass( 'layerSelected' ) ){

						$( obj.layerElement ).addClass('layerSelected');
		...
		container.rotation = _edited.rotation;
			...
			for( var i=0; i < toolsToAttach.components.length; i++ ){
				...
				switch( toolsToAttach.components[i].positionInfo ){
					...
					default:
								//			console.log( _edited.trueWidth/2 );
						if( toolsToAttach.components[i].positionInfo instanceof Object ){
							...
							toolsToAttach.components[i].setPosition( _edited.trueWidth/2,(( _edited.scaleY < 0  ) ? 40 : -40 ) * 1/_edited.scaleY );
