...
.value('taOptions',  {
	...
	toolbar: [
		...
		['html', 'insertImageUpload', 'insertLink', 'insertVideo']
	],
	classes: {
		focussed: "focussed",
		toolbar: "btn-toolbar",
...
//				selector: <string> a jqLite or jQuery selector string customAttribute: <string> an attribute to search for
...
.constant('taTranslations', {
	...
	insertImageUpload: {
		dialogPrompt: 'Wybierz plik do uploadu',
...
.run(['taRegisterTool', '$window', '$modal', 'taTranslations', 'taSelection', function(taRegisterTool, $window, $modal, taTranslations, taSelection){
	taRegisterTool("html", {
	...
	taRegisterTool('insertImageUpload', {
          iconclass: "fa fa-picture-o",
          action: function() {
          	//console.log('upload');
            ...
            var modalInstance = $modal.open({
              ...
              templateUrl: 'views/modalboxes/textAngularUpload.html',
              controller: ['$modalInstance', '$scope', '$filter', 'FileUploader', 'UploadService', 'Notification', '$config',
                function($modalInstance, $scope, $filter, FileUploader, UploadService, Notification, $config) {
                	var uploader = $scope.uploader = new FileUploader({
						    		'url': UploadService.getTAUploadUrl()
				        	});
						...
						uploader.filters.push({
                    		...
                    		fn: function(item, options) {
                      			...
                      			var checkType = '|jpg|png|webp|jpeg|bmp|gif|svg|svg+xml|'.indexOf(type) !== -1;
                      			if(!checkType) {
					        ...
					        uploader.onSuccessItem = function(fileItem, response, status, headers) {
					            var imgUrl = $config.STATIC_URL + response.path;
					            $modalInstance.close();
					            return textAngular.$editor().wrapSelection('insertImage', imgUrl);
              ...
              windowClass: 'taUpload-modal-window'
            });
