...
class RAEClient:
    ...
    def __init__(self, kernel_url: Optional[str] = None, api_key: Optional[str] = None):
        self.kernel_url = kernel_url or os.getenv("RAE_KERNEL_URL", "http://rae-kernel:8000")
        self.api_key = api_key or os.getenv("RAE_AGENT_KEY", "dev-key")
        self._session = requests.Session()
        self._session.headers.update({
            "X-RAE-Agent-Key": self.api_key,
    ...
    def ask(self, intent_name: str, **kwargs) -> KernelResponse:
        ...
        try:
            resp = self._session.post(
                f"{self.kernel_url}/v1/agent/ask",
    ...
    def save_memory(self, content: str, importance: float = 0.5) -> bool:
        ...
        try:
            resp = self._session.post(f"{self.kernel_url}/v1/agent/memory", json=payload)
