...
class UpsDelivery extends Component
{
    ...
    public function shipment(array $preparedData, $packageWeight)
    {
        ...
        if ($preparedData['senderID'] == SENDER_CLIENT) {
            ...
            if (strlen($preparedData['shipFromAddress']['companyName']) > 0) {
                $shipperMainName = $preparedData['shipFromAddress']['companyName'];
            } else {
                $shipperMainName = $preparedData['shipFromAddress']['name'] . ' ' . $preparedData['shipFromAddress']['lastname'];
            }

            $shipFromAddressLine = $preparedData['shipFromAddress']['street'] . ' ' . trim($preparedData['shipFromAddress']['house']);
            if (strlen($preparedData['shipFromAddress']['apartment']) > 0) {
                $shipFromAddressLine .= '/' . trim($preparedData['shipFromAddress']['apartment']);
            }

            $shipper = array(
                'Name' => $shipperMainName,
                'AttentionName' => $shipperAttentionName,
                'TaxIdentificationNumber' => str_replace('-', '', trim($senderData['nip']['value'])),
                'Phone' => array(
                    'Number' => trim($preparedData['shipFromAddress']['telephone'])
                ),
                ...
                'Address' => array(
                    ...
                    'PostalCode' => str_replace('-', '', trim($preparedData['shipFromAddress']['zipcode'])),
        ...
        if ($preparedData['senderID'] == SENDER_CLIENT) {
            if ($preparedData['shipFromAddress']['countryCode'] == 'pl') {
                $charSet = 'pol';
            }
        } else {
            if (strtolower($senderData['country']['value']) == 'pl') {
                $charSet = 'pol';
            }
        }
        ...
        $prepareRequest = array(
            'UPSSecurity' => array(
                ...
                'ServiceAccessToken' => array(
                    'AccessLicenseNumber' => $preparedData['courier']['settings']['key_api']
                )
            ),
        ...
        if (intval($preparedData['courier']['settings']['ups_cod']) == 1) {
        ...
        */
        ...
        if (isset($preparedData['courier']['settings']['ups_test_mode']) && $preparedData['courier']['settings']['ups_test_mode'] == 2) {
            $upsUrl = UPS_SHIP_URL;
        } else {
            $upsUrl = UPS_SHIP_TEST_URL;
        }
        curl_setopt($ch, CURLOPT_URL, $upsUrl . 'rest/Ship');
    ...
    private function getSenderData()
    {
        ...
        return $this->Setting->getAllByModule();
    }
     ...
     * @throws Exception
     */
    private function upsLabelRecovery($courierData, $trackingNumber)
    {
        $request = array(
            "UPSSecurity" => array(
                "UsernameToken" => array(
        ...
        curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 5);
        curl_setopt($ch, CURLOPT_TIMEOUT, 45);
        curl_setopt($ch, CURLOPT_URL, "https://wwwcie.ups.com/rest/LBRecovery");
        curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);
        curl_setopt($ch, CURLOPT_SSLVERSION, CURL_SSLVERSION_TLSv1_2);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    ...
    public function printLabel($base64, $output)
    {
        ...
        try {
            ...
            $this->debug('file not written!', $exception->getMessage());
        }

        return $output;
    }

    public function cancelShipment($preparedData, $shipment)
    {

        $ch = curl_init();

        $headers = array();
        $headers[] = 'transId:' . rand(0, 1000);
        $headers[] = 'transactionSrc:client';
        $headers[] = 'Username:' . $preparedData['courier']['settings']['login'];
        $headers[] = 'Password:' . $preparedData['courier']['settings']['password'];
        $headers[] = 'AccessLicenseNumber:' . $preparedData['courier']['settings']['key_api'];

        $shipmentIdNumber = $shipment['trackingNumber'];
        ...
        curl_setopt($ch, CURLOPT_URL, $upsUrl . 'ship/v1/shipments/cancel/' . $shipmentIdNumber);
        curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);
        curl_setopt($ch, CURLOPT_SSLVERSION, CURL_SSLVERSION_TLSv1_2);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
