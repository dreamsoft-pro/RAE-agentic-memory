...
def ingest_batches(memories, batch_size=500):
    print(
        f"🚀 Starting ingestion of {len(memories)} memories in batches of {batch_size}..."
    )
    ...
    for i in range(0, total, batch_size):
        ...
        print(
            f"📦 Processing batch {i//batch_size + 1}/{(total//batch_size)+1} ({len(batch)} items)..."
        )
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = [
                executor.submit(
                    send_single_memory, session, target_url, m, api_key, tenant_id
                )
                for m in batch
            ]
        ...
        processed += len(batch)
        print(
            f"✅ Batch complete. Progress: {processed}/{total}. Total Errors: {errors}"
        )
...
def prepare():
    ...
    ultra_data["description"] = (
        "Industrial Ultra (100k) with 200 queries for Bandit convergence"
    )
