...
### 2. **Cluster Management (Sync Scripts)**

When working with Node 1 (Lumina), ALWAYS use the dedicated sync scripts to prevent overwriting secrets (.env) or heavy assets (models/).

```bash
# ✅ Push code TO Node 1 (Safe)
./scripts/sync_to_node1.sh

# ✅ Pull code FROM Node 1 (Safe)
./scripts/sync_from_node1.sh
```

### 3. **NEVER use interactive git commands**
...
### 3. **Fail-Fast Testing Strategy (MANDATORY)**

To save time and tokens, we enforce a **Fail-Fast** strategy:
- Tests MUST stop on the first failure (`-x`).
- Fix the error immediately before proceeding.
- Resume tests from the failure point (`--lf` or `--nf`) if possible to avoid re-running passed tests.
...
make test-lite  # Runs everything even if failures occur
```
