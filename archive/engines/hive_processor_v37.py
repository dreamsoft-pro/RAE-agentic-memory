...
def process_hive():
    try:
        ...
        for mem in memories:
            ...
            match = re.search(r"\.factory\(['"]([a-zA-Z0-9]+)['"]", source)
            if not match:
                match = re.search(r"\.service\(['"]([a-zA-Z0-9]+)['"]", source)
            s_name = match.group(1) if match else f'Service_{mid[:8]}'
            
            print(f'[WRITER] Modernizuje z DeepSeek: {s_name}')
...
if __name__ == "__main__":
    ...
    print('HIVE V3.7 (DEEPSEEK EDITION) START')
    while True:
        process_hive()
        time.sleep(5)
