...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint(
            "metric_type IN ('gauge', 'counter', 'histogram')",
            name="metrics_timeseries_valid_type",
        ),
    )
    op.create_index(
        "idx_metrics_ts_metric_tenant_time",
        "metrics_timeseries",
        ["metric_name", "tenant_id", sa.text("timestamp DESC")],
        unique=False,
    )
    op.create_index(
...
def downgrade() -> None:
    ...
    op.drop_index(
        "idx_metrics_ts_tags", table_name="metrics_timeseries", postgresql_using="gin"
    )
