...
class PostgresValidator:
    ...
    async def _get_columns(self, conn: asyncpg.Connection, table_name: str) -> Dict[str, str]:
        ...
        return {row['column_name']: row['udt_name'] if row['udt_name'] else row['data_type'] for row in rows}

    def _validate_columns(self, entity: EntityContract, existing_columns: Dict[str, str]) -> List[ValidationViolation]:
        violations = []
    ...
    def _check_type_compatibility(self, expected: DataType, actual: str) -> bool:
        ...
        mapping = {
            ...
            DataType.FLOAT: ["float4", "float8", "numeric", "real", "double precision"],
            DataType.BOOLEAN: ["bool", "boolean"],
            DataType.TIMESTAMP: ["timestamp", "timestamptz", "date"],
