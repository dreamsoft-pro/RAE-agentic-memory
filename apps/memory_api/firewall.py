...
class SemanticFirewall:
    ...
    def audit_violation(reason: str, intent: str, params: dict):
        ...
        """
        logger.critical(
            f"🛡️  AUDIT VIOLATION: [{intent}] -> {reason} | Params: {params}"
        )
    ...
    def validate_intent(intent_name: str, parameters: dict):
        ...
        for keyword in SemanticFirewall.FORBIDDEN_KEYWORDS:
            if keyword in param_str:
                SemanticFirewall.audit_violation(
                    f"Injection Attempt ({keyword})", intent_name, parameters
                )
                raise HTTPException(
        ...
        if parameters.get("memory_count", 0) > 100:
            logger.info(
                "📉 FIREWALL: Enforcing Memory Gating (100+ items requested). Truncating."
            )
