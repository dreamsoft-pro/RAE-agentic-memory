...
class ReflectionScoring(BaseModel):
    ...
    novelty_score: float = Field(0.5, ge=0.0, le=1.0, description="How novel/unique is this reflection")
    importance_score: float = Field(0.5, ge=0.0, le=1.0, description="How important/significant")
    utility_score: float = Field(0.5, ge=0.0, le=1.0, description="How useful/actionable")
    confidence_score: float = Field(0.5, ge=0.0, le=1.0, description="Model confidence in reflection")
...
class ReflectionTelemetry(BaseModel):
    ...
    generation_model: Optional[str] = Field(None, description="Model used for generation")
    ...
    generation_cost_usd: Optional[float] = Field(None, description="Generation cost in USD")
...
class ReflectionUnit(BaseModel):
    ...
    # Classification
    type: ReflectionType = Field(ReflectionType.INSIGHT, description="Type of reflection")
    priority: int = Field(3, ge=1, le=5, description="Priority level 1-5 (5=highest)")

    # Scoring
    score: float = Field(0.5, ge=0.0, le=1.0, description="Overall reflection quality score")
    scoring: Optional[ReflectionScoring] = Field(None, description="Detailed component scores")

    # Hierarchical structure
    parent_reflection_id: Optional[UUID] = Field(None, description="Parent reflection for hierarchy")
    depth_level: int = Field(0, ge=0, description="Depth in hierarchy (0=base, 1=meta, etc.)")

    # Source tracking
    source_memory_ids: List[UUID] = Field(default_factory=list, description="Source memory IDs")
    source_reflection_ids: List[UUID] = Field(default_factory=list, description="Source reflection IDs")
    ...
    cluster_centroid: Optional[List[float]] = Field(None, description="Cluster centroid vector")
    ...
    # Telemetry
    telemetry: Optional[ReflectionTelemetry] = Field(None, description="Generation telemetry")
...
class ReflectionRelationship(BaseModel):
    ...
    relation_type: ReflectionRelationType = Field(..., description="Type of relationship")
    ...
    # Evidence
    supporting_evidence: List[str] = Field(default_factory=list, description="Supporting evidence texts")
...
class ReflectionCluster(BaseModel):
    ...
    # Summary
    cluster_summary: Optional[str] = Field(None, description="Summary of cluster themes")
    dominant_themes: List[str] = Field(default_factory=list, description="Dominant themes in cluster")
...
class GenerateReflectionRequest(BaseModel):
    ...
    reflection_type: Optional[ReflectionType] = Field(None, description="Desired reflection type")
    max_memories: int = Field(50, gt=0, le=1000, description="Maximum memories to process")
    min_cluster_size: int = Field(3, gt=0, le=50, description="Minimum cluster size")

    # Hierarchical parameters
    parent_reflection_id: Optional[UUID] = Field(None, description="Parent reflection for meta-insights")
    enable_clustering: bool = Field(True, description="Enable clustering-based generation")

    # Filters
    memory_filters: Optional[Dict[str, Any]] = Field(None, description="Filters for memory selection")
    since: Optional[datetime] = Field(None, description="Only consider memories after this timestamp")
...
class QueryReflectionsRequest(BaseModel):
    ...
    # Query parameters
    query_text: Optional[constr(min_length=1, max_length=1024)] = Field(None, description="Semantic query text")
    k: int = Field(10, gt=0, le=100, description="Number of results to return")

    # Filters
    reflection_types: Optional[List[ReflectionType]] = Field(None, description="Filter by reflection types")
    min_priority: Optional[int] = Field(None, ge=1, le=5, description="Minimum priority")
...
class QueryReflectionsResponse(BaseModel):
    ...
    reflections: List[ReflectionUnit] = Field(default_factory=list, description="Matching reflections")
...
class GetReflectionGraphRequest(BaseModel):
    ...
    relation_types: Optional[List[ReflectionRelationType]] = Field(None, description="Filter by relation types")
...
class GetReflectionGraphResponse(BaseModel):
    ...
    nodes: List[ReflectionUnit] = Field(default_factory=list, description="Reflection nodes")
    edges: List[ReflectionRelationship] = Field(default_factory=list, description="Relationships")
...
class ReflectionUsageLog(BaseModel):
    ...
    # Usage context
    usage_type: str = Field(..., description="Type of usage: query, api_call, agent_execution, etc.")
    query_text: Optional[str] = Field(None, description="Query that retrieved this reflection")
