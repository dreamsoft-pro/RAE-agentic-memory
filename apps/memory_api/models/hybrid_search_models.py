...
class QueryAnalysis(BaseModel):
    ...
    key_entities: List[str] = Field(default_factory=list, description="Named entities in query")
    key_concepts: List[str] = Field(default_factory=list, description="Core concepts")
    temporal_markers: List[str] = Field(default_factory=list, description="Time-related terms")
    relation_types: List[str] = Field(default_factory=list, description="Relationship types")
    ...
    suggested_depth: Optional[int] = Field(None, description="Suggested traversal depth")
...
class SearchResultItem(BaseModel):
    ...
    # Composite score
    hybrid_score: float = Field(..., ge=0.0, le=1.0, description="Weighted composite score")
    rerank_score: Optional[float] = Field(None, ge=0.0, le=1.0, description="LLM re-ranking score")
    ...
    source_nodes: List[UUID] = Field(default_factory=list, description="Graph nodes if applicable")
...
class HybridSearchRequest(BaseModel):
    ...
    graph_max_depth: int = Field(3, gt=0, le=5, description="Max depth for graph traversal")
    graph_start_nodes: Optional[List[str]] = Field(None, description="Specific start nodes")
