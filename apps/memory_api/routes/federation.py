...
async def federation_query(request: FederationQueryRequest, req: Request, pool=Depends(get_pool)):
    ...
    with tracer.start_as_current_span("rae.api.federation.query") as span:
        ...
        logger.info("federation_query_received", tenant_id=request.tenant_id, query=request.query_text)

        try:
            # Use RAECoreService via app state
            if not hasattr(req.app.state, "rae_core_service"):
                raise HTTPException(status_code=503, detail="RAE Core Service not available")
            ...
            raise HTTPException(status_code=500, detail=f"Federation query failed: {str(e)}")