...
def test_sync_mesh_peers_task_execution():
    ...
    with patch(
        "apps.memory_api.tasks.background_tasks.rae_context", side_effect=mock_context
    ):
        with patch(
            "apps.memory_api.services.mesh_service.MeshService"
        ) as mock_mesh_service_cls:
...
def test_api_trigger_peer_sync(test_client):
    ...
    mock_mesh_service.redis_client.set.assert_called_once_with(
        "mesh_sync_test-node", "1", ex=300, nx=True
    )
    ...
    stats_data = [
        {
            "status": "success",
            "count": 150,
            "last_sync": datetime(2026, 7, 20, 8, 0, 0),
        },
