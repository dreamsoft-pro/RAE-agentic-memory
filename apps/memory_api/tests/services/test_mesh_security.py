...
from apps.memory_api.api.v2.mesh import (
    receive_sync_data,
)
...
async def test_data_classification_filtering_before_transmission(mesh_service):
    ...
    await mesh_service.register_peer(
        peer_id, "No Consent Peer", "http://peer-nc", "token-123"
    )
    ...
    assert (
        "4" in filtered_ids_consent
    )  # Restricted memory should be allowed since consent grant exists
...
async def test_receiver_sync_handling_classification_guards(mesh_service):
    ...
    payload_unrestricted = {
        ...
        "memories": [
            {"id": "1", "content": "some public info", "info_class": "public"}
        ],
    }
    resp = await receive_sync_data(
        make_mock_request(payload_unrestricted), mesh_service
    )
    ...
    payload_restricted_no_token = {
        ...
        "receiver_id": "rae-host",
        "memories": [
            {"id": "2", "content": "secret email content", "info_class": "restricted"}
        ],
    }
    with pytest.raises(HTTPException) as excinfo:
        await receive_sync_data(
            make_mock_request(payload_restricted_no_token), mesh_service
        )
    ...
    resp_ok = await receive_sync_data(
        make_mock_request(payload_restricted_with_token), mesh_service
    )
...
async def test_db_persistence_calls():
    ...
    service_db = MeshService(
        pool=mock_pool, secret_key="my-test-secret-key-that-is-long-enough-32-chars"
    )

    # Mock decrypt_auth_data to bypass actual decryption in db retrieval test
    with patch.object(
        service_db, "decrypt_auth_data", return_value=("token-abc", "pubkey-xyz")
    ):
        ...
        await service_db.register_peer(
            "peer-2", "New Peer", "http://peer-2", "token-2", "pubkey-2", "grant-456"
        )
