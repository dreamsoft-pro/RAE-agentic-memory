...
async def test_embedding_service_distributed_routing():
    ...
    with patch("apps.memory_api.services.ml_service_client.MLServiceClient") as MockClient:
...
async def test_embedding_service_local_fallback():
    ...
    with patch.object(service, "generate_embeddings", return_value=[[0.4, 0.5, 0.6]]) as mock_sync:
        # Execute
        texts = ["local"]
        result = await service.generate_embeddings_async(texts)
