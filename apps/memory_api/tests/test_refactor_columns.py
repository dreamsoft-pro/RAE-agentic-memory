...
async def test_store_memory_canonical_fields(mock_pool):
    ...
    for arg in args:
        if hasattr(arg, 'isoformat') and not isinstance(arg, str): # simple check for datetime
             # created_at and last_accessed_at are also datetimes.
             # expires_at should be in the future.
             # We can check if any datetime > now + 3500s
             pass

    # 6. Verify Metadata Cleanliness (Phase 2 Requirement: "Usunąć logikę upychani")
    # Find the JSON metadata argument
    import json
    metadata_arg = None
    for arg in args:
