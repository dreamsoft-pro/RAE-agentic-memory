...
def calculate_recency_score(memory: ScoredMemoryRecord, half_life_hours: int = 24) -> float:
    """
    ...
    memory_ts = memory.timestamp if memory.timestamp.tzinfo else memory.timestamp.replace(tzinfo=timezone.utc)
