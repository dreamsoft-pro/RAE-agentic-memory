...
class LLMEvaluationResponse(BaseModel):
    ...
    outcome_type: str = Field(
        ..., description="Outcome type (success, failure, partial, error)"
    )
    reasons: List[str] = Field(..., description="List of reasons for the assessment")
    should_reflect: bool = Field(
        ..., description="Whether this execution warrants reflection"
    )
    importance: float = Field(
        ..., ge=0.0, le=1.0, description="Suggested importance for reflection"
    )
