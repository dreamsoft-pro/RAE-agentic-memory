...
class CommunityDetectionService:
    ...
    async def run_community_detection_and_summarization(
        self, project: str, tenant_id: str
    ):
        """
        ...
        logger.info(
            "starting_community_detection", project=project, tenant_id=tenant_id
        )

        # 1. Load Graph
        graph = await self._load_graph_from_db(project, tenant_id)
        ...
        for community_id, node_ids in communities.items():
            ...
            await self._process_community(
                community_id, node_ids, graph, project, tenant_id
            )
    ...
    async def _process_community(
        ...
        await self._store_super_node(
            community_id, summary, node_ids, project, tenant_id
        )
    ...
    async def _store_super_node(
        ...
        super_node_id = f"community_{project}_{community_id}"
    ...
    async def _load_graph_from_db(self, project: str, tenant_id: str) -> nx.Graph:
        """
