...
class ContextCache:
    ...
    def get_context(self, tenant_id: str, project: str, cache_type: str) -> Optional[str]:
        ...
        if compressed_data:
            metrics.cache_hit_counter.labels(tenant_id=tenant_id, project=project, cache_type=cache_type).inc()
            return ZSTD_DECOMPRESSOR.decompress(compressed_data).decode('utf-8')
        metrics.cache_miss_counter.labels(tenant_id=tenant_id, project=project, cache_type=cache_type).inc()
        return None

    def set_context(self, tenant_id: str, project: str, cache_type: str, data: str) -> None:
        cache_key = self._build_cache_key(tenant_id, project, cache_type)
        ...
        metrics.cache_size_gauge.labels(cache_type=cache_type, project=project).set(len(compressed_data) / (1024 * 1024))
...
async def rebuild_full_cache():
    ...
    semantic_records = await pool.fetch("SELECT tenant_id, project, content FROM memories WHERE layer = 'sm'")
    reflective_records = await pool.fetch("SELECT tenant_id, project, content FROM memories WHERE layer = 'rm'")
