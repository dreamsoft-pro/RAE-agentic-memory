...
class CostOverview(BaseModel):
    ...
    top_tenants: List[dict] = Field(default_factory=list, description="Top 5 tenants by cost")
    top_models: List[dict] = Field(default_factory=list, description="Top 5 models by usage")
...
async def get_governance_overview(
    ...
    **Permissions:** Admin only
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get governance overview: {str(e)}")
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            if row['total_calls'] == 0:
                raise HTTPException(status_code=404, detail=f"No data found for tenant {tenant_id}")
        ...
        async with pool.acquire() as conn:
            ...
            if budget_usd:
                if current_cost >= budget_usd:
                    alerts.append(f"Budget exceeded: ${current_cost:.2f} / ${budget_usd:.2f}")
                    status = "EXCEEDED"
                elif current_cost >= budget_usd * 0.9:
                    alerts.append(f"Critical: 90% of budget used (${current_cost:.2f} / ${budget_usd:.2f})")
                    status = "CRITICAL"
                elif current_cost >= budget_usd * 0.75:
                    alerts.append(f"Warning: 75% of budget used (${current_cost:.2f} / ${budget_usd:.2f})")
                    status = "WARNING"

                if projected_cost > budget_usd:
                    alerts.append(f"Projected to exceed budget: ${projected_cost:.2f} estimated by month end")
                    if status == "OK":
                        status = "WARNING"

            if budget_tokens and current_tokens >= budget_tokens:
                alerts.append(f"Token budget exceeded: {current_tokens:,} / {budget_tokens:,}")
                status = "EXCEEDED"
