...
def test_resolve_entities(mock_clustering, mock_st):
    ...
    with patch("apps.ml_service.services.entity_resolution.cosine_similarity", return_value=[[1.0]]):
        response = client.post("/resolve-entities", json={
...
def test_extract_triples_simple():
    ...
    response = client.post("/extract-triples", json={
        "text": "Bob loves Alice",
        "method": "simple"
    })
